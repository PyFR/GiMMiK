...
from gimmik.ctypesutil import platform_libname
from gimmik.nputil import npdtype_to_ctypestype
from gimmik.util import digest, lazyprop, mv, rm
...
class SourceModule(object):
    ...
    def __init__(self, src, cfg):
        ...
        if not self.mod:
            ...
            tmpdir = tempfile.mkdtemp(prefix='gimmik-{0}-'.format(tmpidx))
            ...
            try:
                ...
                if 'GIMMIK_DEBUG_OMP_KEEP_LIBS' not in os.environ:
                    rm(tmpdir)
    ...
    @lazyprop
    def cachedir(self):
        return os.environ.get('GIMMIK_OMP_CACHE_DIR',
                              user_cache_dir('gimmik', 'gimmik'))

    def _cache_loadlib(self):
        # If caching is disabled then return
        if 'GIMMIK_DEBUG_OMP_DISABLE_CACHE' in os.environ:
            return
