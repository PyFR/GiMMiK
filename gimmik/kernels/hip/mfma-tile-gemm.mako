...
%    endif
            c_old_${j}_${t}_${reg} = load_c(&c[row*ldc + col]);
        }
% endfor
...
%    if guarded:
            if (c_valid_${j}_${t}_${reg})
%    endif
            store_c(&c[row*ldc + col], c_old_${j}_${t}_${reg} + ${c_epilogue_acc_expr(j, t, reg)});
        }
% endfor
</%def>
...
% else:
% if beta == 0:
                store_c(&c[row*ldc + col], ${c_epilogue_acc_expr(j, t, reg)});
% else:
                store_c(&c[row*ldc + col], ${beta}*load_c(&c[row*ldc + col]) + ${c_epilogue_acc_expr(j, t, reg)});
% endif
% endif
        }
%   endfor
