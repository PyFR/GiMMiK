                         ...
                         .param .u64 _c)
{
    ...
    .reg .${pftype} bv_a<${len(bix)}>, bv_b<${len(bix)}>, dotp_a, dotp_b;
    .reg .pred p1;
...
% for i, kx in enumerate(bix):
    ld.weak.global.cg.v2.${pftype} {bv_a${i}, bv_b${i}}, [b_base + ${ldb*kx*dwidth_i}];
% endfor
...
%   for kx, jx in row_nz[j]:
    fma.rn.${pftype} dotp_a, bv_a${bix[kx]}, ${jx}, dotp_a;
    fma.rn.${pftype} dotp_b, bv_b${bix[kx]}, ${jx}, dotp_b;
%   endfor
%   if beta_zero:
...
%   if beta_zero:
    {
        .reg .${pftype} _z;
        mov.${pftype} _z, ${fzero};
        st.weak.global.cg.v2.${pftype} [c_base + ${ldc*j*dwidth_i}], {_z, _z};
    }
%   elif beta != 1:
    {
        .reg .${pftype} _ca, _cb;
