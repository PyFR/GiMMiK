...
%       if needs_zero_init:
            ...
            mov.${pftype} ${c1}, ${fzero};
%       endif
            ${pred_emit(f'ld.weak.global.cg.{pftype} {c0}, [caddr];', pm, pvc0, pred_reg=f'p0_{wm}_{nt}_{mt}_{mg}', indent=' ' * 12)}
            ${pred_emit(f'ld.weak.global.cg.{pftype} {c1}, [caddr + {dwidth_i}];', pm, pvc1, pred_reg=f'p1_{wm}_{nt}_{mt}_{mg}', indent=' ' * 12)}
        }
%      endfor
%     endif
%    endfor
...
%>
        {
            .reg .u64 caddr;
            add.u64 caddr, c_thr_base, ${mt * c_mtile_stride + mg * c_mgroup_stride + nt * c_ntile_stride};
            ${pred_emit(f'st.weak.global.{pftype} [caddr], {c0};', pm, pvc0, pred_reg=f'p0s_{wm}_{nt}_{mt}_{mg}', indent=' ' * 12)}
            ${pred_emit(f'st.weak.global.{pftype} [caddr + {dwidth_i}], {c1};', pm, pvc1, pred_reg=f'p1s_{wm}_{nt}_{mt}_{mg}', indent=' ' * 12)}
        }
%     endfor
%    endfor
%   endfor
