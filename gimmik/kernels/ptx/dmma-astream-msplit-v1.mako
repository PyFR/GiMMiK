...
%   for nt in range(nn):
%    for mt in owned_mts:
        .reg .${pftype} c0_${nt}_${mt}, c1_${nt}_${mt};
%    endfor
%   endfor
%   for mt in owned_mts:
%    if pm_runtime(mt):
...
%      if needs_zero_init:
            ...
            mov.${pftype} c1_${nt}_${mt}, ${fzero};
%      endif
            ${pred_emit(f'ld.weak.global.cg.{pftype} c0_{nt}_{mt}, [caddr];', pm, pvc0, pred_reg=f'p0_{wm}_{nt}_{mt}', indent=' ' * 12)}
            ${pred_emit(f'ld.weak.global.cg.{pftype} c1_{nt}_{mt}, [caddr + {dwidth_i}];', pm, pvc1, pred_reg=f'p1_{wm}_{nt}_{mt}', indent=' ' * 12)}
        }
%     endif
%    endfor
%   endfor
...
%>
        {
            .reg .u32 baddr;
            add.u32 baddr, b_thr_base, ${ki * b_smem_kiter_stride + nt * b_smem_ntile_stride};
%     if needs_zero:
...
%     endif
            ${pred_emit(f'ld.shared.{pftype} b_frag_{nt}, [baddr];', pbrow, pvb, pred_reg=f'pb_{wm}_{ki}_{nt}', indent=' ' * 12)}
        }
%    endfor
%    for mt in owned_mts:
...
%>
        {
            .reg .u64 caddr;
            add.u64 caddr, c_thr_base, ${mt * c_mtile_stride + nt * c_ntile_stride};
            ${pred_emit(f'st.weak.global.{pftype} [caddr], c0_{nt}_{mt};', pm, pvc0, pred_reg=f'p0s_{wm}_{nt}_{mt}', indent=' ' * 12)}
            ${pred_emit(f'st.weak.global.{pftype} [caddr + {dwidth_i}], c1_{nt}_{mt};', pm, pvc1, pred_reg=f'p1s_{wm}_{nt}_{mt}', indent=' ' * 12)}
        }
%    endfor
%   endfor
    }
